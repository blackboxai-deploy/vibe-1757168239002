...
export default function MenPage() {
  ...
  const allMenProducts = getProductsByCategory('men', selectedCategory || undefined);
  
  const filteredProducts = useMemo(() => {
    let products = allMenProducts.filter(product => {
      // Price filter
    ...
    return products;
  }, [allMenProducts, sortBy, priceRange, selectedSizes, selectedColors]);

  const categoryName = selectedCategory 
    ? categories.men.find(cat => cat.id === selectedCategory)?.name 
    : null;
        ...
        <div className="container mx-auto px-4 py-16">
          <div className="max-w-4xl mx-auto text-center">
            <h1 className="text-4xl md:text-6xl font-bold mb-4">
              {categoryName ? `Men's ${categoryName}` : "Men's Collection"}
            </h1>
            <p className="text-xl md:text-2xl text-gray-300 mb-8">
              {categoryName 
                ? `Discover our premium selection of men's ${categoryName.toLowerCase()}`
                : "Sophisticated style meets modern comfort. Discover premium menswear for every occasion."
              }
            </p>
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4">
              <div>
                <h2 className="text-2xl font-bold text-gray-900">
                  {categoryName ? `Men's ${categoryName}` : "Men's Clothing"}
                </h2>
